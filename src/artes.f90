...
contains
   ...
   subroutine radiative_transfer
      ...
      real(dp)    :: ref_normal(3), ref_normal_new(3), stokes_rot(4), surface_normal(3)
      ...
      do i = 1, packages
         ...
         ! Print progress of number of simulated photons
         ...
         ! Initiate Stokes vector as I = 1 and Q = U = V = 0
         ...
         ! Initiate the reference normal (see Sect. 3.7 in Peest et al. 2017)
         ! Perpendicular to propagation direction

         call init_ref_normal(direction, ref_normal)

         ! Peel a thermally emitted photon

         if (photon_source .eq. 2) then

            ! Thermal planet photon (unpolarized)
         ...
         ! Calculate optical depth to the edge of the grid
         ...
         do
            ...
            ! Exit do loop if inner or outer edge is reached
         ...
         ! Sample the first optical depth

         if (next_face(1) .eq. 1 .and. next_face(2) .eq. cell_depth .and. surface_albedo .gt. 0._dp) then

            ! Photon may hit the planet surface and gets reflected
            ! Sample an optical depth without weighting to make sure that the photon can reach the planet surface.
            ...
            if (tau_first .ge. 1.e-6_dp) then
               ...
               ! The optical depth to the grid edge is larger than 1e-6

               ! Sample first optical depth with randum number generator
               call random(thread_id, xi)

               if (tau_first .lt. 50._dp) then

                  ! Force the first interaction and weight the Stokes vector
                  tau = -log(1._dp - xi*(1._dp - exp(-tau_first)))
                  stokes = stokes*(1._dp - exp(-tau_first))

               else

                  ! If tau_first > 50 then calculate separately because exp(-tau_first) is very small
                  tau = -log(1._dp - xi)
               ...
               ! The optical depth to the grid edge is smaller than 1e-6

               ! Start a new photon if it reaches the outer edge of the grid (i.e. grazing incidence)
               cycle
         ...
         ! Do loop to start crossing cells untill the next interaction point
         ...
         do
            ...
            if (tau_run + tau_cell .gt. tau) then
               ...
               ! Interaction in current cell so exit do loop
               ...
               ! No interaction yet so move photon to cell boundary
            ...
            ! Photon is crossing the inner grid boundary (i.e. planet surface)
            ...
            if (next_face(1) .eq. 1 .and. next_face(2) .eq. cell_depth) then
               ...
               if (xi .gt. surface_albedo) then
                  ...
                  ! Photon is absorbed by surface
                  surface = .true.
                  exit

               else if (xi .le. surface_albedo) then

                  ! Photon is reflected by surface
                  call lambertian(thread_id, x, y, z, stokes, direction, ref_normal, surface_normal)
                  call peel_surface(thread_id, i, x, y, z, stokes, cell, face, surface_normal)
            ...
            ! Keep track of the total optical depth
            tau_run = tau_run + tau_cell

         end do

         ! Keep track of the total flux leaving the atmosphere
         ...
         ! Continue scattering the photon until it either exits the atmosphere or is absorbed

         do

            ! Exit do loop if scattering is turned off with thermal photons
            if (.not. photon_scattering) exit

            ! Sample random number to determine scattering or absorption
            call random(thread_id, xi)
            ...
            if (xi .lt. fstop) then
               ...
               if (cell_albedo(cell(1), cell(2), cell(3), wl_count) .lt. 1._dp) then

                  ! Weight the Stokes vector because of fstop parameter
               ...
               ! Project the peeled Stokes vector on the detector
               call peel_photon(thread_id, i, x, y, z, stokes, direction, cell, face, ref_normal, cell_error)

               ! Stop photon if error has occurred
               if (cell_error) exit

               ! Sample scattering direction
               call sample_scattering(thread_id, stokes, cell, theta_scat, beta_scat, cos_theta_scat, c2b, s2b)

               ! Get Mueller matrix
               call get_mueller(theta_scat, cell, scat_matrix)

               ! Calculate the new reference normal
               call new_reference(beta_scat, direction, ref_normal, ref_normal_new)

               ! Calculate the new photon direction
               call new_direction(cos_theta_scat, direction, ref_normal_new, direction_new)

               ! Rotate Stokes vector
               call rotate_stokes(stokes, c2b, s2b, stokes_rot)

               ! Scatter Stokes vector
               call scatter_stokes(stokes_rot, scat_matrix, .false., stokes_new)

               ! Set updated Stokes vector, direction vector, and reference normal

               stokes = stokes_new
               direction = direction_new
               ref_normal = ref_normal_new
            ...
            if (photon_walk .gt. 0._dp .and. face(1) .eq. 0) then
               ...
               if (r_min .gt. photon_walk/cell_opacity(cell(1), cell(2), cell(3), wl_count)) then
                  ...
                  call isotropic_direction(thread_id, direction)
            ...
            if (.not. walk) then
               ...
               do
                  ...
                  ! Cross cells untill the sampled optical depth is reached or the photon leaves the grid
                  ...
                  if (tau_run + tau_cell .gt. tau) then
                     ! Next interaction point is in the current grid cell
                     ...
                     ! Photon crosses a cell face before reaching the next interaction point
                     ...
                     ! Update the last cell face that is crossed and the current cell
                     face = next_face
                     cell = cell_out
                  ...
                  ! Photon crosses the inner grid boundary (i.e. planet surface)
                  ...
                  if (next_face(1) .eq. 1 .and. next_face(2) .eq. cell_depth) then
                     ...
                     if (xi .gt. surface_albedo) then
                        ...
                        ! Photon gets absorbed by planet surface
                        ...
                        ! Photon reflects from planet surface
   ...
   subroutine emit_photon(thread_id, x_emission, y_emission, z_emission, emission_direction, &
      ...
      if (photon_source .eq. 1) then
         ...
         if (photon_emission .eq. 1) then
            ...
            ! Isotropic
            call isotropic_direction(thread_id, emission_direction)
...
!             call direction_cosine(-alpha, beta, cos(2._dp*beta), radial_unit, emission_direction)
   ...
   subroutine init_ref_normal(direction, ref_normal)

      ! Initiate the reference normal, perpendicular to the direction vector

      real(dp), intent(in)  :: direction(3)
      real(dp), intent(out) :: ref_normal(3)

      if (direction(3) .lt. 1._dp - 1.e-10_dp) then

         ref_normal(1) = -direction(1)*direction(3)/sqrt(1._dp - direction(3)**2)
         ref_normal(2) = -direction(2)*direction(3)/sqrt(1._dp - direction(3)**2)
         ref_normal(3) = sqrt(1._dp - direction(3)**2)
   ...
   end subroutine init_ref_normal

   subroutine lambertian(thread_id, x, y, z, stokes, direction, ref_normal_new, surface_normal)
      ...
      real(dp), intent(out)   :: direction(3), ref_normal_new(3), surface_normal(3)
      real(dp)                :: xi, cos_theta, beta, ref_normal(3)

      ! Surface normal on spherical surface at point of reflection and scale for oblate surface
      ! The surface normal can be considered as the input direction vector
      ...
      ! Reinitiate the reference normal.
...
!       cos_theta = xi
      ...
      call new_direction(cos_theta, surface_normal, ref_normal_new, direction)
   ...
   subroutine sample_scattering(thread_id, stokes, cell_in, theta, beta, cos_theta, c2b, s2b)

      integer               :: i
      integer, intent(in)   :: thread_id, cell_in(3)
      real(dp), intent(in)  :: stokes(4)
      real(dp), intent(out) :: theta, beta, cos_theta, c2b, s2b
      real(dp)              :: intensity(180), intensity_cum(0:180), intensity_cum_sampled, xi
      ...
      do i = 1, 180
         ...
         intensity(i) = cell_p11_int(cell_in(1), cell_in(2), cell_in(3), wl_count)*stokes(1) + &
                        cell_p12_int(cell_in(1), cell_in(2), cell_in(3), wl_count)*stokes(2)*cos2beta(i) + &
                        cell_p12_int(cell_in(1), cell_in(2), cell_in(3), wl_count)*stokes(3)*sin2beta(i) - &
                        cell_p13_int(cell_in(1), cell_in(2), cell_in(3), wl_count)*stokes(2)*sin2beta(i) + &
                        cell_p13_int(cell_in(1), cell_in(2), cell_in(3), wl_count)*stokes(3)*cos2beta(i) + &
                        cell_p14_int(cell_in(1), cell_in(2), cell_in(3), wl_count)*stokes(4)
...
!       end if
      ...
      do i = 1, 180
         ...
         intensity(i) = cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, 1, i)*stokes(1) + &
                        cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, 2, i)*c2b*stokes(2) + &
                        cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, 2, i)*s2b*stokes(3) - &
                        cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, 3, i)*s2b*stokes(2) + &
                        cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, 3, i)*c2b*stokes(3) + &
                        cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, 4, i)*stokes(4)
      ...
      do i = 1, 180
         ...
         if (intensity_cum_sampled .ge. intensity_cum(i - 1) .and. intensity_cum_sampled .le. intensity_cum(i)) then
            ...
            theta = (dble(i) - dble(i - 1))*(intensity_cum_sampled - intensity_cum(i - 1))/ &
                    (intensity_cum(i) - intensity_cum(i - 1)) + dble(i - 1)
   ...
   subroutine get_mueller(theta_scat, cell_in, mueller_matrix)
      ...
      if (mod(theta_scat*180._dp/pi, 1._dp) .gt. 0.5_dp) then

         angle_upper = int(theta_scat*180._dp/pi) + 2
         angle_lower = int(theta_scat*180._dp/pi) + 1

      else

         angle_upper = int(theta_scat*180._dp/pi) + 1
         angle_lower = int(theta_scat*180._dp/pi)

      end if

      ! Interpolate mueller matrix

      if (angle_upper .eq. 1) then

         mueller_matrix = cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, :, 1)

      else if (angle_lower .eq. 180) then

         mueller_matrix = cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, :, 180)

      else

         x0 = dble(angle_lower) - 0.5_dp
         x1 = dble(angle_upper) - 0.5_dp

         x_inter = theta_scat*180._dp/pi

         y0 = cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, :, angle_lower)
         y1 = cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, :, angle_upper)

         mueller_matrix = y0 + (x_inter - x0)*(y1 - y0)/(x1 - x0)

!          do i = 1, 16
!
!             y0 = cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, i, angle_lower)
!             y1 = cell_scatter_matrix(cell_in(1), cell_in(2), cell_in(3), wl_count, i, angle_upper)
!
!             mueller_matrix(i) = y0 + (x_inter - x0)*(y1 - y0)/(x1 - x0)
!
!          end do

      end if

   end subroutine get_mueller

   subroutine new_reference(beta_scat, direction_in, ref_normal, ref_normal_new)

      ! Calculate the new reference normal

      real(dp), intent(in)  :: beta_scat, direction_in(3), ref_normal(3)
      real(dp), intent(out) :: ref_normal_new(3)
      real(dp)              :: dir_cross_ref(3), cos_beta_scat, sin_beta_scat

      cos_beta_scat = cos(beta_scat)
      sin_beta_scat = sin(beta_scat)

      dir_cross_ref(1) = direction_in(2)*ref_normal(3) - direction_in(3)*ref_normal(2)
      dir_cross_ref(2) = direction_in(3)*ref_normal(1) - direction_in(1)*ref_normal(3)
      dir_cross_ref(3) = direction_in(1)*ref_normal(2) - direction_in(2)*ref_normal(1)

      ref_normal_new(1) = ref_normal(1)*cos_beta_scat + dir_cross_ref(1)*sin_beta_scat
      ref_normal_new(2) = ref_normal(2)*cos_beta_scat + dir_cross_ref(2)*sin_beta_scat
      ref_normal_new(3) = ref_normal(3)*cos_beta_scat + dir_cross_ref(3)*sin_beta_scat

      ref_normal_new = ref_normal_new/sqrt(ref_normal_new(1)**2 + ref_normal_new(2)**2 + ref_normal_new(3)**2)

   end subroutine new_reference

   subroutine new_direction(cos_theta_scat, direction_in, ref_normal_new, direction_new)

      ! Calculate the new direction vector

      real(dp), intent(in)  :: cos_theta_scat, direction_in(3), ref_normal_new(3)
      real(dp), intent(out) :: direction_new(3)
      real(dp)              :: ref_new_cross_dir(3), sin_theta_scat

      sin_theta_scat = sqrt(1._dp - cos_theta_scat**2)

      ref_new_cross_dir(1) = ref_normal_new(2)*direction_in(3) - ref_normal_new(3)*direction_in(2)
      ref_new_cross_dir(2) = ref_normal_new(3)*direction_in(1) - ref_normal_new(1)*direction_in(3)
      ref_new_cross_dir(3) = ref_normal_new(1)*direction_in(2) - ref_normal_new(2)*direction_in(1)

      direction_new(1) = direction_in(1)*cos_theta_scat + ref_new_cross_dir(1)*sin_theta_scat
      direction_new(2) = direction_in(2)*cos_theta_scat + ref_new_cross_dir(2)*sin_theta_scat
      direction_new(3) = direction_in(3)*cos_theta_scat + ref_new_cross_dir(3)*sin_theta_scat

      direction_new = direction_new/sqrt(direction_new(1)**2 + direction_new(2)**2 + direction_new(3)**2)

   end subroutine new_direction

   subroutine rotate_stokes(stokes_in, c2b, s2b, stokes_rot)

      real(dp), intent(in)  :: stokes_in(4), c2b, s2b
      real(dp), intent(out) :: stokes_rot(4)
      real(dp)              :: norm

      ! Rotate the Stokes vector to the new reference normal

      stokes_rot(1) = stokes_in(1)
      stokes_rot(2) = c2b*stokes_in(2) + s2b*stokes_in(3)
      stokes_rot(3) = -s2b*stokes_in(2) + c2b*stokes_in(3)
      ...
      if (sqrt(stokes_rot(2)**2 + stokes_rot(3)**2 + stokes_rot(4)**2) .gt. 0._dp) then
         ...
         norm = sqrt(stokes_in(2)**2 + stokes_in(3)**2 + stokes_in(4)**2)/ &
                sqrt(stokes_rot(2)**2 + stokes_rot(3)**2 + stokes_rot(4)**2)
      ...
      stokes_rot = stokes_rot*norm

   end subroutine rotate_stokes

   subroutine scatter_stokes(stokes_in, scat_matrix, peeling, stokes_scat)

      logical, intent(in)   :: peeling
      real(dp), intent(in)  :: stokes_in(4), scat_matrix(16)
      real(dp), intent(out) :: stokes_scat(4)

      ! Apply scattering matrix (i.e. the Mueller matrix)

      stokes_scat(1) = scat_matrix(1)*stokes_in(1) + scat_matrix(2)*stokes_in(2) + &
                       scat_matrix(3)*stokes_in(3) + scat_matrix(4)*stokes_in(4)

      stokes_scat(2) = scat_matrix(5)*stokes_in(1) + scat_matrix(6)*stokes_in(2) + &
                       scat_matrix(7)*stokes_in(3) + scat_matrix(8)*stokes_in(4)

      stokes_scat(3) = scat_matrix(9)*stokes_in(1) + scat_matrix(10)*stokes_in(2) + &
                       scat_matrix(11)*stokes_in(3) + scat_matrix(12)*stokes_in(4)

      stokes_scat(4) = scat_matrix(13)*stokes_in(1) + scat_matrix(14)*stokes_in(2) + &
                       scat_matrix(15)*stokes_in(3) + scat_matrix(16)*stokes_in(4)

      ! Normalize the stokes vector such that the total intensity is conserved

      if (.not. peeling .and. stokes_scat(1) .gt. 0._dp) then

         stokes_scat = stokes_scat*stokes_in(1)/stokes_scat(1)
         ...
         do j = 0, ntheta - 1
            do k = 0, nphi - 1
               do m = 1, n_wavelength
                  ...
                  if (cell_opacity(i, j, k, m) .gt. 0._dp) cell_albedo(i, j, k, m) = &
                     cell_scattering_opacity(i, j, k, m)/cell_opacity(i, j, k, m)

                  if (cell_albedo(i, j, k, m) .lt. 1.e-10_dp) then

                     cell_albedo(i, j, k, m) = 1.e-10_dp

                  else if (cell_albedo(i, j, k, m) .gt. 1._dp) then

                     write (6, *) ""
                     write (6, *) "ERROR: The single scattering albedo is greater than 1."
                     call exit(0)
            ...
            do k = 0, nphi - 1
               do m = 1, n_wavelength
                  do n = 1, 180
                     ...
                     cell_p11_int(i, j, k, m) = cell_p11_int(i, j, k, m) + &
                                                cell_scatter_matrix(i, j, k, m, 1, n)*sinbeta(n)*pi/180._dp

                     cell_p12_int(i, j, k, m) = cell_p12_int(i, j, k, m) + &
                                                cell_scatter_matrix(i, j, k, m, 2, n)*sinbeta(n)*pi/180._dp

                     cell_p13_int(i, j, k, m) = cell_p13_int(i, j, k, m) + &
                                                cell_scatter_matrix(i, j, k, m, 3, n)*sinbeta(n)*pi/180._dp

                     cell_p14_int(i, j, k, m) = cell_p14_int(i, j, k, m) + &
                                                cell_scatter_matrix(i, j, k, m, 4, n)*sinbeta(n)*pi/180._dp
      ...
      if (mode .eq. 1) then
               ...
               do i = 0, nr - 1
                  ...
                  if (nphi .eq. 1) then
                     ...
                     if (k .lt. nphi - 1) then
                        ...
                        cell_volume(i, j, k) = oblate_x*oblate_y*oblate_z*(1._dp/3._dp)* &
                                               (rfront(i + 1)**3 - rfront(i)**3)*(theta_grid_cos(j) - theta_grid_cos(j + 1))* &
                                               (phifront(k + 1) - phifront(k))
                  ...
                  do k = 0, nphi - 1
                     ...
                     if (cell_temperature(i, j, k) .gt. 0._dp .and. cell_absorption_opacity(i, j, k, wl_count) .gt. 0._dp) then
                        ...
                        if (thermal_weight) then
                           ...
                           cell_weight(i, j, k) = weight_norm/(cell_volume(i, j, k)* &
                                                               cell_absorption_opacity(i, j, k, wl_count)*planck_flux)
                        ...
                        cell_luminosity(i, j, k) = 4._dp*pi*cell_volume(i, j, k)* &
                                                   cell_absorption_opacity(i, j, k, wl_count)*planck_flux ! [W m-1]
         ...
         if (det_type .eq. 2) then
            ...
            do i = 0, nr - 1
               ...
               optical_depth_total = optical_depth_total + &
                                     ((rfront(i + 1) - rfront(i))*cell_opacity(i, 0, 0, wl_count))

               optical_depth_total1 = optical_depth_total1 + &
                                      ((rfront(i + 1) - rfront(i))*cell_scattering_opacity(i, 0, 0, wl_count))

               optical_depth_total2 = optical_depth_total2 + &
                                      ((rfront(i + 1) - rfront(i))*cell_absorption_opacity(i, 0, 0, wl_count))
   ...
   subroutine cell_face(photon, x, y, z, n, face, next_face, face_distance, grid_exit, cell, cell_out, cell_error)
      ...
      ! Calculate the distance to the nearest cell face

      ! The grid_min parameter is used to remove photons for which the minimum distance
      ! to a cell face is smaller than grid_min. Not enitrely clear if this parameter
      ! is really needed.
      ...
      ! Check what the inner, same, and outer boundaries are

      if (face(1) .eq. 1) then

         ! Current face is a radial face
         ...
         ! Current face is an azimuthal face
   ...
   subroutine write_output
      ...
      if (det_type .eq. 3) then
         ...
         else if (det_phi*180._dp/pi .gt. 179._dp) then
   ...
   subroutine output(log_number)
      ...
      if (log_number .eq. 1) then
         ...
         if ((photon_source .eq. 2) .and. (thermal_weight)) then
            write (out_unit, '(a)') "Weighting thermal emission: on"
         else if ((photon_source .eq. 2) .and. (.not. thermal_weight)) then
            write (out_unit, '(a)') "Weighting thermal emission: off"
         end if
         write (out_unit, '(a)') ""
         write (out_unit, '(a,es8.2)') "Maximum vertical optical depth: ", tau_max
         write (out_unit, '(a)') ""
         if (det_type .eq. 1 .or. det_type .eq. 3) then
   ...
   subroutine argument_keywords
      ...
      do i = 1, command_argument_count()
         ...
         case ('-k')
            ...
            open (100, file=trim(output_name)//'/input/'//adjustl(trim(input_file)), &
   ...
   subroutine peel_thermal(thread_id, photon, x_photon, y_photon, z_photon, stokes_peel_in, cell_in, face_in, cell_error)
      ...
      ! Peel a thermal photon when it is emitted
      ...
      do
         ...
         call cell_face(photon, x, y, z, det_dir, face, next_face, face_distance, grid_exit, cell, cell_out, cell_error)

         tau_cell = face_distance*cell_opacity(cell(1), cell(2), cell(3), wl_count)
         tau_total = tau_total + tau_cell

         x = x + face_distance*det_dir(1)
         y = y + face_distance*det_dir(2)
         z = z + face_distance*det_dir(3)
      ...
      if (grid_exit .and. tau_total .lt. 50._dp .and. .not. cell_error) then
         ...
         if (photon_weight*stokes_peel_in(1) .gt. 0._dp .and. photon_weight*stokes_peel_in(1) .lt. 1.e100_dp) then
            ...
            detector_thread(ix, iy, 1, 1, thread_id + 1) = &
               detector_thread(ix, iy, 1, 1, thread_id + 1) + photon_weight*stokes_peel_in(1)

            detector_thread(ix, iy, 1, 2, thread_id + 1) = &
               detector_thread(ix, iy, 1, 2, thread_id + 1) + (photon_weight*stokes_peel_in(1))**2

            detector_thread(ix, iy, 1, 3, thread_id + 1) = &
               detector_thread(ix, iy, 1, 3, thread_id + 1) + 1._dp
   ...
   subroutine peel_surface(thread_id, photon, x_photon, y_photon, z_photon, stokes_peel_in, cell_in, &
                           face_in, surface_normal)

      integer, intent(in)     :: cell_in(3), face_in(2), thread_id
      integer(16), intent(in) :: photon
      integer                 :: cell(3), cell_out(3), face(2), next_face(2), ix, iy
      real(dp), intent(in)    :: x_photon, y_photon, z_photon, stokes_peel_in(4), surface_normal(3)
      real(dp)                :: x, y, z, face_distance, photon_weight
      real(dp)                :: tau_cell, tau_total, x_im, y_im, cos_angle, x_new, y_new
      logical                 :: grid_exit, cell_error

      ! Determine angle between surface normal and detector direction

      cos_angle = surface_normal(1)*det_dir(1) + surface_normal(2)*det_dir(2) + surface_normal(3)*det_dir(3)

      ! Copy cartesian coordinates into new parameters for use in do loop
      ...
      if (cos_angle .gt. 0._dp) then
         ...
         ! Projection on the detector is only possible if the angle is smaller than 90 deg
         ...
         if (grid_exit .and. tau_total .lt. 50._dp .and. .not. cell_error) then
            ...
            ! Lambertian scattering probability:
            ! P(mu,phi) dmu dphi = 2*mu*dmu*dphi/(2pi) -> P(mu,phi) = mu/pi
            ...
            if (photon_weight*stokes_peel_in(1) .gt. 0._dp .and. photon_weight*stokes_peel_in(1) .lt. 1.e100_dp) then
               ...
               ! Photon is unpolarized so only add Stokes I
   ...
   subroutine peel_photon(thread_id, photon, x_in, y_in, z_in, stokes_peel_in, direction_in, &
                          cell_in, face_in, ref_normal, cell_error)
      ...
      ! Copy global cartesian detector direction into new array for local use

      det(1) = det_dir(1)
      det(2) = det_dir(2)
      det(3) = det_dir(3)

      ! Calculate optical depth to grid edge
      ...
      ! Check if the photons is leaving the grid and the optical depth is smaller than 50

      if (.not. cell_error .and. grid_exit .and. tau_total .lt. 50._dp) then

         ! Weighting factor for the Stokes vector

         photon_weight = exp(-tau_total)

         ! Cosine of the scattering angle (i.e. dot product of two direction vectors)

         cos_theta_obs = direction_in(1)*det(1) + direction_in(2)*det(2) + direction_in(3)*det(3)

         if (cos_theta_obs .ge. 1._dp) then
            cos_theta_obs = 1._dp - 1.e-10_dp
         else if (cos_theta_obs .le. -1._dp) then
            cos_theta_obs = -1._dp + 1.e-10_dp
         end if

         ! New reference normal

         ref_normal_obs(1) = direction_in(2)*det(3) - direction_in(3)*det(2)
         ref_normal_obs(2) = direction_in(3)*det(1) - direction_in(1)*det(3)
         ref_normal_obs(3) = direction_in(1)*det(2) - direction_in(2)*det(1)

         ref_normal_obs = ref_normal_obs/sqrt(ref_normal_obs(1)**2 + ref_normal_obs(2)**2 + ref_normal_obs(3)**2)

         ! Azimuthal rotation angle

         if (cos_theta_obs .gt. 1._dp - 1.e-8_dp) then

            ! The photon direction is parallel to the detector direction so phi_obs is undefined/
            ! The scattering plane does not change so simply set phi_obs = 0.
            ...
            cos_phi_obs = ref_normal(1)*ref_normal_obs(1) + ref_normal(2)*ref_normal_obs(2) + ref_normal(3)*ref_normal_obs(3)

            ref_cross(1) = ref_normal(2)*ref_normal_obs(3) - ref_normal(3)*ref_normal_obs(2)
            ref_cross(2) = ref_normal(3)*ref_normal_obs(1) - ref_normal(1)*ref_normal_obs(3)
            ref_cross(3) = ref_normal(1)*ref_normal_obs(2) - ref_normal(2)*ref_normal_obs(1)

            sin_phi_obs = ref_cross(1)*direction_in(1) + ref_cross(2)*direction_in(2) + ref_cross(3)*direction_in(3)

            if (sin_phi_obs .ge. 1._dp) then
               sin_phi_obs = 1._dp - 1.e-10_dp
            else if (sin_phi_obs .le. -1._dp) then
               sin_phi_obs = -1._dp + 1.e-10_dp
            end if

         end if

         ! Rotate Stokes vector by the azimuthal scattering angle

         ! https://en.wikipedia.org/wiki/List_of_trigonometric_identities#Double-angle_formulae
         s2b = 2._dp*sin_phi_obs*cos_phi_obs
         c2b = cos_phi_obs**2 - sin_phi_obs**2

         call rotate_stokes(stokes_peel_in, c2b, s2b, stokes_rot)

         ! Get Mueller matrix

         call get_mueller(acos(cos_theta_obs), cell_in, scatter)

         ! Apply Mueller matrix to the rotated Stokes vector

         call scatter_stokes(stokes_rot, scatter, .true., stokes_out)

         ! Rotate Stokes vector into reference frame of the detector

         ref_north(1) = -cos_det_theta*cos_det_phi
         ref_north(2) = -cos_det_theta*sin_det_phi
         ref_north(3) = sin_det_theta

         ref_east(1) = det(2)*ref_north(3) - det(3)*ref_north(2)
         ref_east(2) = det(3)*ref_north(1) - det(1)*ref_north(3)
         ref_east(3) = det(1)*ref_north(2) - det(2)*ref_north(1)

         cos_alpha_obs = ref_normal_obs(1)*ref_east(1) + ref_normal_obs(2)*ref_east(2) + ref_normal_obs(3)*ref_east(3)

         if (cos_alpha_obs .ge. 1._dp) then
            cos_alpha_obs = 1._dp - 1.e-10_dp
         else if (cos_alpha_obs .le. -1._dp) then
            cos_alpha_obs = -1._dp + 1.e-10_dp
         end if

         ref_cross(1) = ref_normal_obs(2)*ref_east(3) - ref_normal_obs(3)*ref_east(2)
         ref_cross(2) = ref_normal_obs(3)*ref_east(1) - ref_normal_obs(1)*ref_east(3)
         ref_cross(3) = ref_normal_obs(1)*ref_east(2) - ref_normal_obs(2)*ref_east(1)

         sin_alpha_obs = ref_cross(1)*det(1) + ref_cross(2)*det(2) + ref_cross(3)*det(3)

         if (sin_alpha_obs .ge. 1._dp) then
            sin_alpha_obs = 1._dp - 1.e-10_dp
         else if (sin_alpha_obs .le. -1._dp) then
            sin_alpha_obs = -1._dp + 1.e-10_dp
         end if

         s2b = 2._dp*sin_alpha_obs*cos_alpha_obs
         c2b = cos_alpha_obs**2 - sin_alpha_obs**2

         call rotate_stokes(stokes_out, c2b, s2b, stokes_obs)

         ! Project photon on detector
         ...
         if (photon_source .eq. 2) then
            ...
            ! Not clear why a minus sign is needed for the thermal photons but it
            ! gives the correct Stokes vectors in the images.

            stokes_obs(2) = -1._dp*stokes_obs(2)
            stokes_obs(3) = -1._dp*stokes_obs(3)

         end if

         if (photon_weight*stokes_obs(1) .gt. 0._dp .and. photon_weight*stokes_obs(1) .lt. 1.e100_dp) then

            detector_thread(ix, iy, 1, 1, thread_id + 1) = &
               detector_thread(ix, iy, 1, 1, thread_id + 1) + photon_weight*stokes_obs(1)

            detector_thread(ix, iy, 2, 1, thread_id + 1) = &
               detector_thread(ix, iy, 2, 1, thread_id + 1) + photon_weight*stokes_obs(2)

            detector_thread(ix, iy, 3, 1, thread_id + 1) = &
               detector_thread(ix, iy, 3, 1, thread_id + 1) + photon_weight*stokes_obs(3)

            detector_thread(ix, iy, 4, 1, thread_id + 1) = &
               detector_thread(ix, iy, 4, 1, thread_id + 1) + photon_weight*stokes_obs(4)

            detector_thread(ix, iy, 1, 2, thread_id + 1) = &
               detector_thread(ix, iy, 1, 2, thread_id + 1) + (photon_weight*stokes_obs(1))**2

            detector_thread(ix, iy, 2, 2, thread_id + 1) = &
               detector_thread(ix, iy, 2, 2, thread_id + 1) + (photon_weight*stokes_obs(2))**2

            detector_thread(ix, iy, 3, 2, thread_id + 1) = &
               detector_thread(ix, iy, 3, 2, thread_id + 1) + (photon_weight*stokes_obs(3))**2

            detector_thread(ix, iy, 4, 2, thread_id + 1) = &
               detector_thread(ix, iy, 4, 2, thread_id + 1) + (photon_weight*stokes_obs(4))**2

            detector_thread(ix, iy, 1, 3, thread_id + 1) = &
               detector_thread(ix, iy, 1, 3, thread_id + 1) + 1._dp

            detector_thread(ix, iy, 2, 3, thread_id + 1) = &
               detector_thread(ix, iy, 2, 3, thread_id + 1) + 1._dp

            detector_thread(ix, iy, 3, 3, thread_id + 1) = &
               detector_thread(ix, iy, 3, 3, thread_id + 1) + 1._dp

            detector_thread(ix, iy, 4, 3, thread_id + 1) = &
               detector_thread(ix, iy, 4, 3, thread_id + 1) + 1._dp
   ...
   subroutine isotropic_direction(thread_id, direction)

      ! Sample isotropic direction vector

      integer, intent(in)   :: thread_id
      real(dp), intent(out) :: direction(3)
      real(dp)              :: xi, cos_theta, sin_theta, phi, cos_phi, sin_phi

      call random(thread_id, xi)
      cos_theta = 2._dp*xi - 1._dp
      sin_theta = sqrt(1._dp - cos_theta**2)
