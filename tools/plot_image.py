...
pol_frac = np.zeros((npix, npix))

for i in range(npix):
    for j in range(npix):
        if stokes_i[i, j] > 0.:
            pol_frac[i, j] = np.sqrt(stokes_q[i, j]**2+stokes_u[i, j]**2)/stokes_i[i, j]

pol_ang = 0.5*np.arctan2(stokes_u, stokes_q)
...
cb = plt.colorbar(fig)
cb.ax.set_ylabel('Surface brightness (W m$^{-2}$ µm$^{-1}$ mas$^{-2}$)')
...
fig = plt.imshow(stokes_q, origin='lower', cmap='twilight', interpolation='nearest',
                 vmin=Qmin, vmax=Qmax, extent=[-npix/2.,npix/2.,-npix/2.,npix/2.])
...
fig = plt.imshow(stokes_u, origin='lower', cmap='twilight', interpolation='nearest',
                 vmin=Umin, vmax=Umax, extent=[-npix/2.,npix/2.,-npix/2.,npix/2.])
...
fig = plt.imshow(error_i_frac, origin='lower', cmap='magma', interpolation='nearest',
                 vmin=0.0, vmax=error_i_max, extent=[-npix/2.,npix/2.,-npix/2.,npix/2.])
...
for i in range(npix):
    for j in range(npix):
        if pol_frac[i,j] > 0.:
            error_p_frac[i,j] = error_p[i, j]/pol_frac[i, j]

error_p_max = np.nanmax(error_p_frac)
if error_p_max > 1.:
    error_p_max = 1.

fig = plt.imshow(error_p_frac, origin='lower', cmap='magma', interpolation='nearest',
                 vmin=0.0, vmax=error_p_max, extent=[-npix/2.,npix/2.,-npix/2.,npix/2.])
...
fig = plt.imshow(stokes_i, origin='lower', cmap='gist_heat', interpolation='nearest',
                 vmin=0., vmax=np.amax(stokes_i), extent=[-npix/2.,npix/2.,-npix/2.,npix/2.])
...
plt.quiver(npix/15.-npix/2., npix/20.-npix/2., 1., 0., scale=scaling/(label_pol/100.),
           width=0.007, pivot='middle', headwidth=0, headlength=0, headaxislength=0,
           color='#66CCFF', edgecolor='#66CCFF')

plt.annotate(label, xy=(npix/15.-npix/2., npix/20.-npix/2.), xycoords='data', color='white',
...
plt.annotate(polDeg, xy=(0,0), xycoords='axes fraction', xytext=(0.97,0.97),
             textcoords='axes fraction', color='white', weight='bold', ha='right', va='top')
